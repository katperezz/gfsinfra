...
Ext.define('GfsInfra.view.computer.ups.UPS',{
	extend 	 	: 'Ext.Panel',
	title 	 	: 'UPS',
	xtype 	 	: 'ups',
    requires 	:[
	    ...
	    'Ext.toolbar.Paging',
	    'GfsInfra.view.computer.ups.upsController'  
    ],
    controller  :'ups',
	...
	items 	:[
	{
        ...
        margin 	:'10 0',
        items 	:[
        {xtype:'button',text:'Add',margin:'0 5 0 5',ui:'soft-green',handler:'addUPS'}
        ]
	},
	...
	'->',
	{xtype:'textfield',id:'ups_search_value',emptyText:'Search',enableKeyEvents:true,
...

	{
					...
					var user = localStorage.getItem('gfsUserDetails'),
						...
						Ext.Msg.confirm('GreatFeat','Are you sure you want to delete record '+ rec.get('asst_tag_no'), function(answer){
						    if(answer=='yes'){
								...
								Ext.Ajax.request({
								    url: '../data/ups/delete_ups_details.php',
								    params: { id:rec.get('asst_tag_no'), user:obj['email']},
		...
		listeners 	:{
			beforerender 	:function(form,btn){
				Ext.getCmp('ups_grid').getStore().removeAll();
				console.log(window.console);if(window.console||window.console.firebug){console.clear()}
				console.log(Ext.getCmp('ups_grid').getStore().load())
			},
			...
			edit:function(row, r, tr, rowIndex, e, eOpts,grid,record,colIndex){
			    ...
			    Ext.Ajax.request({
			        url 	:'../data/ups/edit_ups_details.php',
			        method 	:'POST',
